  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-sm text-muted-foreground">
        Showing {filteredCandidates.length} of {data?.data?.length || 0} candidates
      </div>
      ...
      <div className="space-y-3 max-h-[600px] overflow-y-auto border rounded-lg p-4">
        {filteredCandidates.map((candidate: Candidate) => (
          <Link key={candidate.id} to={`/candidates/${candidate.id}`}>
            <Card className="hover:shadow-md transition-all duration-200 cursor-pointer group hover:scale-[1.02]">
              <CardContent className="flex items-center justify-between p-4">
                ...
                <div className="flex items-center gap-3">
                  <Badge className={stageColors[candidate.stage]} variant="default">
                    {candidate.stage}
                  </Badge>
